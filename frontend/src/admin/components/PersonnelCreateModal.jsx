...
export default function PersonnelCreateModal(props) {
  if (!props.open) return null;
  return <PersonnelCreateModalInner {...props} />;
}

function PersonnelCreateModalInner({
  onClose,
  onSubmit,
  form,
  setForm,
  ...
  useEffect(() => {
    ...
    const ref = (() => {
      ...
      if (firstErrKey === "password") return passwordRef;
  ...
  return (
    <div className="modal-backdrop-soft" onClick={onClose}>
      <div className="modal-card personnel-create-modal" onClick={(e) => e.stopPropagation()}>
        <button className="modal-close" onClick={onClose}>
          ×
        </button>
        <div className="service-create-header">
          <span className="service-create-icon" aria-hidden="true" />
          <div className="service-create-title">Ajouter Personnel</div>
        </div>

        <form className="modal-body-scroll" onSubmit={onSubmit} noValidate>
          <div className="subsection-title">Renseignements du Personnel</div>
          ...
          <div className="profile-grid mb-3">
            ...
            <div className="profile-main">
              <div className="profile-name-heading">{displayName || "Nouveau Personnel"}</div>
          ...
          <div className="subsection-title">Accès à la Plateforme</div>
          <div className="row g-3 mb-2 password-row align-items-start">
            <div className="col-md-4 password-options-col">
              <label className="form-label d-block mb-0">Mot de Passe *</label>
              <div className="form-check password-auto-check">
                <input
                  className="form-check-input"
                  type="checkbox"
                  id="autoPwdCheck"
                  checked={autoGeneratePwd}
                  onChange={onToggleAutoPwd}
                />
                <label className="form-check-label" htmlFor="autoPwdCheck">
                  Générer automatiquement
                </label>
              </div>
            </div>
            <div className="col-md-8">
              <label className="form-label visually-hidden">Mot de passe *</label>
              <input
                ref={passwordRef}
                className="form-control form-control-sm password-input"
                placeholder="Saisir manuellement ou cocher Générer"
                value={form.password}
                readOnly={autoGeneratePwd}
                onChange={(e) => {
              ...
              {formErrors.password ? <div className="text-danger small mt-1">{formErrors.password}</div> : null}
            </div>
          </div>
          {generatedPwd ? (
            <div className="alert alert-info d-flex justify-content-between align-items-center">
              <div>
                <div className="fw-semibold">Identifiants provisoires</div>
                <div>Matricule : {form.matricule || "—"}</div>
                <div>Mot de passe : {generatedPwd}</div>
              </div>
              <span className="badge bg-light text-dark">CLI*</span>
            </div>
          ) : null}

          <div className="service-create-actions">
            <button className="btn service-create-cancel-btn" onClick={onClose} disabled={saving}>
              Annuler
            </button>
